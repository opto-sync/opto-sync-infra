# OCI registry adapter for `opto-sync/opto-sync-infra`
...
PROMOTE_OCI_DRY_RUN=1 ./scripts/promote-oci-image.sh \
  ...
  registry.example/opto-sync:sha-0123456789ab portable
```

Use OIDC/workload identity or credential helpers. Do not put PATs, access keys, service-account JSON, Docker passwords, or R2 credentials in this repository or Terraform state. Review cost, region, cleanup, IAM, and rollback before apply or Crossplane sync. Production deployment manifests must pin the emitted digest.
