...
function run(command, args, input) {
  ...
  const result = spawnSync(command, args, { input, env, encoding: 'utf8', timeout: 20_000 });
  assert.equal(result.status, 0, `${command} failed: ${result.stderr || result.error}`);
  return result.stdout.trim();
}
for (const provider of ['supabase','neon']) for (const lane of ['auth','admin']) {
  test(`${provider}/${lane}: migration records exact topology and replays idempotently`, { skip: enabled ? false : 'set SHARED_AUTH_TEST_PG=1' }, () => {
    const database = `opto_sync_${provider}_${lane}_${randomUUID().replaceAll('-', '')}`;
    run('createdb', ['--maintenance-db=postgres','--template=template0',database]);
    const sql = readFileSync(join(root, provider, lane, 'migrations/202609070001_shared_auth_runtime_policy.sql'), 'utf8');
    const psql = ['-X','--no-password','--set=ON_ERROR_STOP=1','--dbname',database];
    const admin = lane === 'admin';
    ...
    const query = "SELECT coalesce(jsonb_agg(to_jsonb(p) - 'recorded_at'), '[]'::jsonb) FROM opto_sync.shared_auth_runtime_policy p;";
    for (let replay = 0; replay < 2; replay += 1) {
      run('psql', psql, sql);
      const actual = JSON.parse(run('psql', [...psql,'-At','--command',query]));
