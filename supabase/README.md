...
This directory is non-secret desired state for the dedicated Supabase organization paired with GitHub org `opto-sync`. The previous shared `oresoftware` placement is retired for this topology; provider runtime and target ownership must remain `opto-sync`.
...
- Database URLs, service-role keys, passwords, provider tokens, private keys, and decrypted environment values stay in secret management or encrypted `env/enc`, never source.
